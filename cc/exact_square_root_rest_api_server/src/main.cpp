...
int main() {
    ...
    svr.Get(R"(/exactSquareRoot/(\d+))", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        std::lock_guard<std::mutex> lock(mtx);
        exact_square_root::ExactSquareRoot esr;
        exact_square_root::Res r = esr.GetExactSqrt(id);
        // std::cout << "Calc: " << r.multiplicator << std::endl;
        // std::cout << "Calc: " << r.square_root << std::endl;
        json result{
          {"multiplicator", r.multiplicator},
          {"squareRoot", r.square_root}
        };
